...
export function EditHabitScreen({ id }: { id: string }) {
  ...
  const submit = async (draft: HabitDraft) => {
    try {
      ...
      showError('Não foi possível salvar as alterações.', error);
    }
  };
  ...
  const toggleArchive = async () => {
    try {
      ...
      showError('Não foi possível arquivar o hábito.', error);
    }
  };
  ...
  const handleDelete = async () => {
    const confirmed = await confirm({
      ...
      message: `"${habit.name}" e todo o seu histórico deixarão de aparecer. Se quiser só pausar, arquive o hábito.`,
    ...
    try {
      ...
      showError('Não foi possível excluir o hábito.', error);
    }
  };

  return (
    <Screen edges={['bottom', 'left', 'right']}>
      <HabitForm key={habit.id} initial={initial} submitLabel="Salvar" onSubmit={submit} />
      <View style={styles.actions}>
        <Button
          variant="secondary"
          icon={isArchived ? 'archive-arrow-up-outline' : 'archive-outline'}
          label={isArchived ? 'Desarquivar' : 'Arquivar'}
          accessibilityHint="Arquivar esconde o hábito sem apagar o histórico"
          onPress={toggleArchive}
        />
        <Button variant="danger" icon="trash-can-outline" label="Excluir" onPress={handleDelete} />
      </View>
    </Screen>
  );
}
